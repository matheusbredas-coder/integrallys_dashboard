...
**✅ Gestek Sync is now NATIVE (no n8n) and live-verified.** `src/features/sync/` calls the Gestek API directly and upserts Supabase; matching is on `gestek_id` (TDD), name-collisions are skipped (not inserted), there's an insert guard + dry-run (`/api/sync?dryRun=1`), and it's idempotent. Verified: dry-run + 2 real runs → 0 patients inserted, 156 recent vendas upserted, Clientes stayed 337, revenue R$424.038,95. Spec/plan: `docs/superpowers/{specs,plans}/2026-06-04-native-gestek-sync*`. **`SYNC_ENABLED` is the kill-switch** (gate the button + `/api/sync`). Migration **010** (`clientes_view` computes metrics from `gestek_vendas`) is applied. Env: `GESTEK_API_TOKEN`, `GESTEK_SYNC_MONTHS`=3. The old n8n trigger/webhook code was removed.

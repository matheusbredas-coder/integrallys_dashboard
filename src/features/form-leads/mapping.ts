...
//
// Two payload shapes, both funnelled through `coerceIngestFields`:
//   - a flat `{ label: answer }` object at the top level. This is the live ingest: an n8n
//     workflow reads the "Lead Nova" email, pulls the fields out itself, and POSTs them.
//     Ottokit's retired Facebook Lead Ads payload was the same shape with the answers nested
//     one level down in `field_data`, which is still unpacked.
...
//
// `message_id` is deliberately NOT reserved — it's small, and it's how you trace a stored lead
// back to the email n8n read it out of.
 ...
 * Otherwise the top level *is* the field map: whatever n8n posts (`full_name`, `phone_number`,
 * `id`, ...) is matched against `ALIASES`, and anything unrecognized is still kept in `raw`.
 *
 * Entries nested in `field_data` are merged *over* the top level, so a form question named
 * `campaign_name` can't be shadowed by the campaign it ran under. Only Ottokit ever sent that
 * key; it's kept so replaying an old payload still works.
 */
...
/**
 * The value to dedupe on: Meta's lead id when the payload carried one, otherwise the id of the
 * Gmail message n8n read the lead out of.
 *
 * The fallback is what makes the n8n HTTP node's retries safe when the lead id is missing.
 * Gmail message ids are unique and never reused, so a re-POST of the same lead hits the unique
 * index in migration 021 and settles as `duplicate: true` instead of inserting a second row.
 * Prefixed with `gmail:` so it can never collide with a Meta lead id.
 *

...
export async function runGestekSync(opts: { dryRun?: boolean; trigger?: string }, deps?: RunDeps): Promise<SyncResult> {
  ...
  // Every failure path must stamp the log row: a started_at with no completed_at/error
  // is indistinguishable from a run still in flight, which is how the 01/07 cron death
  // went unnoticed for 18 days.
  const fail = async (code: "gestek_error" | "error", message: string): Promise<SyncResult> => {
    if (!dryRun) await store.logError(run_id, now().toISOString(), message);
    return { ok: false, code, message };
  };
  if (!dryRun) await store.logStart({ run_id, started_at, trigger: opts.trigger ?? "webhook", mode: "sync" });
  let clientes: GestekCliente[];
  try {
    clientes = await gestek.fetchAllClientes();
  } catch (e) {
    return fail("gestek_error", e instanceof Error ? e.message : "Falha ao buscar dados do Gestek.");
  }
  ...
  try {
    ...
    try {
      ...
      return fail("gestek_error", e instanceof Error ? e.message : "Falha ao buscar vendas do Gestek.");
    }
    ...
    try {
      ...
      return fail("gestek_error", e instanceof Error ? e.message : "Falha ao buscar agenda do Gestek.");
    }
    ...
    return fail("error", e instanceof Error ? e.message : "Falha inesperada durante a sincronização.");
  }
}
